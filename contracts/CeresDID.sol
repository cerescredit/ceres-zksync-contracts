...
// File: contracts-zk/interfaces/ICeresDIDStorage.sol
...
// File: contracts-zk/libs/Base64.sol
...
// File: contracts-zk/libs/INFTDescriptor.sol
...
// File: contracts-zk/libs/IERC721.sol
...
// File: contracts-zk/libs/ERC721.sol
...
// File: contracts-zk/libs/ERC721Enumerable.sol
...
// File: contracts-zk/CeresDID.sol
...
contract CeresDID is CfoNftTakeableV2,ERC721Enumerable,NFTMintManager {

    ICeresDIDStorage public immutable didStorage;

    bool private transferAllowed;

    address public tokenDescriptor;
    string private constant description = "Ceres DID is Decentralized digital identity system with programmable metadata and verifiable signatures.";

    event Minted(address minter,address to,uint tokenId,bytes32[] packedProps,string did);

    constructor(
        address _didStorage
    ) ERC721("Ceres DID NFT", "Ceres DID NFT") {

        didStorage = ICeresDIDStorage(_didStorage);
    ...
    function allPropsLength() external view returns(uint){
        return didStorage.allPropsLength();
    }

    function mintedTokenOf(address account) external view returns(uint){
        return didStorage.mintedTokenOf(account);
    }

    function minterOf(uint tokenId) external view returns(address){
        return didStorage.minterOf(tokenId);
    }
    ...
    function safeMint(
        ...
        bytes32[] memory packedProps = didStorage.addItem(tokenId, to, numberProps, stringProps, did, imageUrl);
    ...
    function didOwnerOf(string calldata did) external view returns(address) {
        ...
        return ownerOf(didStorage.didTokenIdOf(did));
    }

    function imageOf(uint tokenId) public view returns(string memory) {
        return didStorage.customImageOf(tokenId);
    }
    ...
    function _propertiesToJson(uint tokenId) internal view returns(string memory){
        ICeresDIDStorage didStorage_ = didStorage; 
        bytes memory output = abi.encodePacked('[{"trait_type":"DID","value":"', didStorage_.tokenDIDOf(tokenId),'"}');
        output = abi.encodePacked(output,',{"trait_type":"Minter","value":"', Strings.toHexString(uint(uint160(didStorage_.minterOf(tokenId))),20),'"}');

        (uint[] memory numberProps,string[] memory stringProps) = didStorage_.itemProperties(tokenId);


        for(uint i=0;i<stringProps.length;i++){
            output = abi.encodePacked(output,',{"trait_type":"',didStorage_.propertyNameOf(i),'","value":"', didStorage_.stringPropertyOf(tokenId,i),'"}'); 
        }

        for(uint i=0;i<numberProps.length; i++){
            string memory aliasName_ = didStorage_.aliasOf(i+stringProps.length, numberProps[i]);
            output = abi.encodePacked(output,',{"trait_type":"',didStorage_.propertyNameOf(i+stringProps.length),'","value":"',(bytes(aliasName_).length > 0 ? aliasName_ : Strings.toString(numberProps[i])),'"}'); 
        }
        output = abi.encodePacked(output,']');
        return string(output);
    }

    function itemProperties(uint tokenId) public view returns(uint[] memory numberProperties,string[] memory stringProperties) {
        return didStorage.itemProperties(tokenId);
    }
    ...
    function updateProperty(uint tokenId,uint mapIndex,uint pos,uint length,uint newVal) external onlyUpdater {
        ...
        didStorage.updateProperty(tokenId, mapIndex, pos,length,newVal);
    }

    function updatePackedProperties(uint tokenId, uint mapIndex, bytes32 newPackedProps) public onlyUpdater {
        require(_exists(tokenId),"updateProperty: token id nonexistent");
        didStorage.updatePackedProperties(tokenId,mapIndex,newPackedProps);
    }
    ...
    function updateCustomImage(uint tokenId, string memory imageUri) external onlyUpdater {
        ...
        didStorage.updateCustomImage(tokenId, imageUri);
    }

    function updateStringProperty(uint tokenId,uint propIndex, string calldata val) external onlyUpdater {
        require(_exists(tokenId),"token id not exists");
        didStorage.updateStringProperty(tokenId, propIndex, val);
    }
    ...
    function didExists(string calldata did) public view returns(bool){
        return didStorage.didExists(did);
    }
